...
# https://github.com/senbox-org/CDSE-Copernicus-DEM-downloader.git 基于这个项目进行补充
...
def check_and_refresh_token(dem_downloader, token_pkg: Dict):
    """
    检查Token是否过期，自动刷新
    token_pkg: {'access': str, 'refresh': str, 'start_time': datetime, 'token_time': datetime}
    """
    now = datetime.now()

    # 超过10分钟，刷新Token
    if (now - token_pkg['token_time']) > timedelta(seconds=600):
        print("   🔄 Token已过期(>10分钟)，正在刷新...")
        try:
            new_access, new_refresh = dem_downloader.refresh_access_token(token_pkg['refresh'])
            token_pkg['access'] = new_access
            token_pkg['refresh'] = new_refresh
            token_pkg['token_time'] = now
            print("   ✅ Token刷新成功")
        except Exception as e:
            print(f"   ⚠️  刷新失败，尝试重新获取: {e}")
            auth = Credentials()
            new_access, new_refresh = dem_downloader.get_access_token(auth.id, auth.password)
            token_pkg['access'] = new_access
            token_pkg['refresh'] = new_refresh
            token_pkg['token_time'] = now

    # 超过60分钟，强制退出（Copernicus硬性限制）
    if (now - token_pkg['start_time']) > timedelta(seconds=3600):
        print("   ⛔ 会话超过60分钟，已达到Copernicus限制，需要重新运行程序")
        raise TimeoutError("Session expired after 60 minutes, please restart the script")

    return token_pkg['access']
    ...
    for google_remote_file in tqdm(tif_files, desc=f"处理 {group_name}"):
        try:
            ...
            for tile_name in required_tiles:
                ...
                access_token = check_and_refresh_token(dem_downloader, token_pkg)
...
def download_copernicus(ref_inputs, output_dirs, resolution="30", dem_format="DGED"):
    ...
    auth = Credentials()
    access_token, refresh_token = dem_downloader.get_access_token(auth.id, auth.password)
    ...
    print(f"🔑 Token获取成功，开始时间: {token_pkg['start_time'].strftime('%H:%M:%S')}")
    print(f"⏱️  注意：Copernicus Token每10分钟自动刷新，60分钟后需重新运行\n")
    ...
    for res_input, output_dir in zip(ref_inputs, output_dirs):
        ...
        try:
            ...
            print(f"\n⚠️  {e}")
            print(f"💡 提示：已下载 {len(downloaded_tiles)} 个瓦片，重新运行脚本可继续下载剩余部分")
            all_complete = False
            break  # 退出循环，保存现有进度

    if all_complete:
        # 创建完成标志
        with open(os.path.join(dem_downloader.dem_directory, ".download_complete"), 'w') as f:
            f.write("All tiles downloaded successfully")
        print("✅ 所有瓦片下载完毕")
        return 88  # 特殊退出码表示全部完成

    return 0  # 正常退出，但可能还有未下载的
